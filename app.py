...
logger.basicConfig(
    format='%(asctime)s\t%(pathname)s\t%(message)s', level=logger.INFO)
...
def get_content(content_url, content_title, content_index):
    ...
    for el in doc(".article-content img").items():
        ...
        img_path = os.path.join(
            IMG_PATH, teg[0][0], teg[0][1], content_index, os.path.basename(img_src))
